...
function add_tags(t){	
	...
	for(var m in inputArr){
		if(inputArr[m]){
			// Get 标签ID
			$.get(site_url +"knowledge/base/insertTag/"+ inputArr[m], function(d){
				if(d>-1){
					insert_id = d;
				}
			});
			...
			// 贴标签 & 改颜色
			$.get(site_url +"knowledge/base/addTag/"+ $('#id').val() +"/"+ insert_id, function(d){
				if(d==1){
...
function quickAddTags(t){	
	...
	// Get 标签ID
	$.get(site_url +"knowledge/base/insertTag/"+ input, function(d){
		if(d>-1){
			insert_id = d;
		}
	});
...
// 删除标签
function del_tags(t){
	console.log(site_url +"knowledge/base/delTag/"+ $('#id').val() +'/'+ $(t).attr('data-tagid'))
	$.get(site_url +"knowledge/base/delTag/"+ $('#id').val() +'/'+ $(t).attr('data-tagid'), function(d){
		if(d==1){
...
function rename(tit) {
	$('body').overhang({
		...
		callback: function () {
			...
			$.post(site_url +"knowledge/handle/rename", obj, function(d){
